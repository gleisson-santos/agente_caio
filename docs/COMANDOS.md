...
| Ação | Comando |
|------|---------|
| **Iniciar gateway** | `$env:PYTHONIOENCODING='utf-8'; python -m caiocore.cli.commands gateway` |
...
```
/root/Caio-Corp/config.json → /root/.caiocore/config.json (dentro do container)
```
