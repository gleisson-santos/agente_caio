...
class AgentRegistry:
    ...
    def register(self, agent) -> None:
        ...
        # The token agent is a specific type of agent, not necessarily passive.
        # The original logic for setting _token_agent based on agent_id is more appropriate.
    ...
    def register_passive(self, agent_id: str, name: str, role: str, tier: int = 2) -> None:
        """
        Register a passive agent (specialist) using the SpecialistAgent class.
        """
        if agent_id not in self._agents:
            from caiocore.agents.specialist import SpecialistAgent
            spec = SpecialistAgent(agent_id, name, role, instruction_file=None, tier=tier)
            self.register(spec)
