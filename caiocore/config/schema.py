...
class EmailConfig(Base):
    ...
    model_config = ConfigDict(populate_by_name=True)  # aceita snake_case E alias

    enabled: bool = False
    consent_granted: bool = Field(False, alias="consentGranted")  # Explicit owner permission to access mailbox data

    # IMAP (receive)
    imap_host: str = Field("", alias="imapHost")
    imap_port: int = Field(993, alias="imapPort")
    imap_username: str = Field("", alias="imapUsername")
    imap_password: str = Field("", alias="imapPassword")
    imap_mailbox: str = Field("INBOX", alias="imapMailbox")
    ...
    smtp_use_ssl: bool = Field(False, alias="smtpUseSsl")
    from_address: str = Field("", alias="fromAddress")

    # Behavior
    auto_reply_enabled: bool = Field(True, alias="autoReplyEnabled")  # If false, inbound email is read but no automatic reply is sent
    poll_interval_seconds: int = Field(30, alias="pollIntervalSeconds")
    mark_seen: bool = Field(True, alias="markSeen")
    max_body_chars: int = Field(12000, alias="maxBodyChars")
    subject_prefix: str = Field("Re: ", alias="subjectPrefix")
    allow_from: list[str] = Field(default_factory=list, alias="allowFrom")  # Allowed sender email addresses
...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.caiocore/workspace"
...
class Config(BaseSettings):
    ...
    def _match_provider(self, model: str | None = None) -> tuple["ProviderConfig | None", str | None]:
        ...
        from caiocore.providers.registry import PROVIDERS, find_by_name
