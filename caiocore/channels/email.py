...
from caiocore.config.schema import EmailConfig
...
class EmailChannel(BaseChannel):
    ...
    def get_recent_emails(self, limit: int = 5) -> list[dict]:
        ...
        return list(reversed(self._recent_emails))[:limit]
    ...
    def remove_from_cache(self, subject: str = "", sender: str = "") -> None:
        ...
        for em in self._recent_emails:
            match = False
            if subject and subject.lower() in em.get("subject", "").lower():
                match = True
            if sender and sender.lower() in em.get("sender", "").lower():
                match = True
                
            if not match:
                new_cache.append(em)
                
        if len(new_cache) < len(self._recent_emails):
            logger.info("Removed {} email(s) from local cache for sender='{}', subject='{}'", 
                        len(self._recent_emails) - len(new_cache), sender, subject)
            self._recent_emails = new_cache
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        base_subject = self._last_subject_by_chat.get(to_addr, "caio reply")
    ...
    def _reply_subject(self, base_subject: str) -> str:
        subject = (base_subject or "").strip() or "caio reply"
