...
from caiocore.agent.tools.email_send import EmailSendTool
from caiocore.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
...
class AgentLoop:
    ...
    async def _connect_mcp(self) -> None:
        ...
        from caiocore.agent.tools.mcp import connect_mcp_servers
        try:
    ...
    async def _process_message(
        ...
        if not target_spec_id:
            ...
            if match:
                target_spec_id = f"spec-{match.group(1).lower()}"
        
        if target_spec_id:
            from caiocore.agents.registry import agent_registry
            spec_agent = agent_registry.get(target_spec_id)
            
            # Fallback for IDs without 'spec-' prefix
            if not spec_agent and not target_spec_id.startswith("spec-"):
                spec_agent = agent_registry.get(f"spec-{target_spec_id}")
                
            if spec_agent and hasattr(spec_agent, "get_instructions"):
                specialist_instruction = spec_agent.get_instructions()
    ...
    async def _call_provider_with_fallback(
        ...
        from caiocore.providers.base import LLMResponse
